...
Adafruit_MQTT_Subscribe disco = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/DiscoSkull");
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(0.01))) {
    valueOperational = atoi((char *)disco.lastread);
  ...
  if(valueOperational == 1){
    Serial.println("ON!");
    delay(15000);
    //uint16_t valueOperational
  }
  
  /*
  if(currentMillis - startMillis <= 15000) { //test whether the maximum fill period has elapsed
    //Turn on
    Serial.println("ON!");
